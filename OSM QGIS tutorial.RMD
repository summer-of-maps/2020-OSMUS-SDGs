---
title: "Downloading and Mapping OSM Data using QGIS"
author: "Eugene Chong"
...
This tutorial is a guide to using QGIS to download and map data from OpenStreetMap. [OpenStreetMap](https://www.openstreetmap.org/), or OSM, is a free, editable map of the world created and edited entirely by volunteer contributors known as mappers. [QGIS](https://qgis.org/en/site/) is a free, open-source geographic information system, or GIS, application that allows users to view, edit, visualize, and analyze geographic data. This tutorial is intended to guide a complete beginner to OpenStreetMap and GIS software through the process of creating a simple, informative map.  
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/install.jpg")`
...
One easy way to add basemaps in QGIS is to use the QuickMapServices **plugin**. Just like your browser, QGIS features a large collection of plugins, which users can install to add additional functionality to the software. To install the _QuickMapServices_ plugin, first go to `Plugins --> Manage and Install Plugins...`.

`r knitr::include_graphics(path = "~plots/~tutorial_images/plugin-1.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/plugin-2.PNG")`

While we're here, let's also install the _QuickOSM_ plugin. This plugin will let us search for and download data from OpenStreetMap. Search for "quickosm" and install that plugin just like you installed QuickMapServices.

`r knitr::include_graphics(path = "~plots/~tutorial_images/plugin-3.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/QuickMapServices-1.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/QuickMapServices-2.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Layers-1.PNG")`

As the name implies, maps in QGIS are made up of layers. The "OSM Standard" layer corresponds to the OSM Standard basemap that we added. Unchecking the box turns the layer "off", meaning we don't see it anymore. When you added the second basemap, that showed up as another layer. The _order_ of layers is very important. Layers will cover up any layers that are underneath. Try clicking and dragging the layers to change their order. Do you see any changes on the map? (Make sure the boxes for your layers are checked when you try this).
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/QuickMapServices-4.PNG")`
`r knitr::include_graphics(path = "~plots/~tutorial_images/QuickMapServices-3.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Stamen-toner-background.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Quick OSM-1.PNG")`

A screen pops up for QuickOSM, and by default it's open on the Quick Query tab. For this tutorial, we'll be sticking with Quick Query and its simple interface. As you continue working with OSM, however, you might find that Quick Query doesn't provide the flexibility you need to download certain datasets. In those cases, you'll want to refer to the Query tab, which lets you write [customized queries](http://overpass-turbo.eu/) to OpenStreetMap's [Overpass API](https://wiki.openstreetmap.org/wiki/Overpass_API).

`r knitr::include_graphics(path = "~plots/~tutorial_images/Quick OSM-2.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Quick OSM-3.PNG")`

Switch the drop-down option back to "In", and take another look at the message. We definitely want to download data for Baltimore, so that part is correct, but we don't want _all_ OSM objects, just the parks and playgrounds. How do we find those? Let's start with the parks.
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Tag-2.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-1.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-2.PNG")`

Click through the table to get a sense of the information available. Notice that many of the fields are empty. While most of the parks have a name recorded in OSM, some of them do not, and mappers will need to add that information in the future. For now, let's scroll down to "Kirk Avenue Athletic Field" and then _select_ it by clicking on the row number (the number "18" in the picture below).

`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-3.PNG")`

Then, right-click on the row, press "Zoom to Feature", and return to the map. 

`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-4.PNG")`

You'll see that one of the points in the center of your map is now shaded yellow (the selected park). Zoom into it.

`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-5.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-6.PNG")`

The satellite image confirms that an athletic field is there, but it hasn't been fully mapped yet. If you'd like, you can contribute to OSM by drawing the park's shape yourself! See ([this link](https://wiki.openstreetmap.org/wiki/Beginners%27_guide)) for a beginner's guide to contributing to OpenStreetMap.
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-7.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-8.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-9.PNG")`

`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-10.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Exercise-1.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-22.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-23.PNG")`
`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-24.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-25.PNG")`

You've just successfully added a column to your data that provides the land area in acres for every park in Baltimore!

`r knitr::include_graphics(path = "~plots/~tutorial_images/Analysis-26.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Styling-1.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Styling-4.PNG")`
`r knitr::include_graphics(path = "~plots/~tutorial_images/Styling-5.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-1.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-2.PNG")`
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-3.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-4.PNG")`
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-5.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-6.PNG")`
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-7.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-8.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-9.PNG")`
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-10.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-11.PNG")`
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-12.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-13.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-14.PNG")`
`r knitr::include_graphics(path = "~plots/~tutorial_images/Print-15.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Export-1.PNG")`
...
`r knitr::include_graphics(path = "~plots/~tutorial_images/Export-2.PNG")`
